...
- *A script that given a function's input types, generates examples of those types and call the function with the generated input values.* I adapted our PBT script from the WakingUp experiment/repo to do this. 
- *A feedback loop.* While to do this in full generality would require making a LeanTool plugin out of the above script, for this initial experiment I decided to simply test it out in Claude Code, which uses the model Claude Sonnet 3.7.
